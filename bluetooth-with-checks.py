...
try:
    # Check initial switch state and set Bluetooth accordingly
    initial_switch_state = GPIO.input(switch_pin)
    initial_bluetooth_state = check_bluetooth_state()

    if initial_switch_state == GPIO.LOW and initial_bluetooth_state:
        set_bluetooth_power(False)
    elif initial_switch_state == GPIO.HIGH and not initial_bluetooth_state:
    ...
    while True:
        ...
        # Check the state of the external switch and Bluetooth
        switch_state = GPIO.input(switch_pin)
        bluetooth_state = check_bluetooth_state()

        if switch_state == GPIO.LOW and bluetooth_state:
            # Switch is off and Bluetooth is on, turn off Bluetooth
            set_bluetooth_power(False)
            time.sleep(1)  # Add a delay to avoid rapid switching

        elif switch_state == GPIO.HIGH and not bluetooth_state:
            # Switch is on and Bluetooth is off, turn on Bluetooth
            set_bluetooth_power(True)
            time.sleep(1)  # Add a delay to avoid rapid switching
